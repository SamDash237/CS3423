...
int concCmd(Cmd cmdM[], int iCmdCount, Token tokenM[], int iTokenCount);
int pipeCmd(Cmd cmdM[], int iCmdCount, Token tokenM[], int iTokenCount);
...
int concCmd(Cmd cmdM[], int iCmdCount, Token tokenM[], int iTokenCount)
{
    ...
    int count = 1;
    int pPid, fPid;
    int cPid[iCmdCount+1];
    int i, j;
    int x = 0;
    int a = 0;
    char *exArgv[20];
    char *cmnd;
    char *cmdlist[iCmdCount+1];
    char *arglist[iCmdCount][iTokenCount];
    int fd[2];
    int flagI;
    int flagO;
    ...
    for(i = 0; i < iCmdCount; i++) {
        ...
        switch(lForkPid)
        {
            ...
            if(cmdM[i].iStdinRedirectIdx != 0) {
                ...
                printf("stdout redirect fileno %d Outtoken %s\n", fd[1], tokenM[cmdM[i].iStdoutRedirectIdx]);
                flagO = 1;
            } else {
